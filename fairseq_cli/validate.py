...
from knnbox.utils import get_registered_datastore, registe_datastore, filter_pad_tokens

MODE = os.environ.get("MODE")
DATASTORE_SAVE_PATH = os.environ.get("DATASTORE_SAVE_PATH")
KEY_DIM = os.environ.get("KEY_DIM")
if MODE == "build_datastore":
    if get_registered_datastore("datastore") is None:
        # create datastore at given path
    ...
    for subset in args.valid_subset.split(","):
        ...
        for i, sample in enumerate(progress):
            ...
            if MODE == "build_datastore":
                ...
                datastore.add_value(non_pad_tokens)
                datastore.set_mask(mask)
            ## <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
if __name__ == "__main__":
    cli_main()
    ## knnbox add code >>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    # dump the datastore and faiss index to disk
    if MODE == "build_datastore":
        datastore.dump()
