:<<! 
[script description]: build a datastore for kernel smoothed knn-mt
[dataset]: multi domain DE-EN dataset
[base model]: WMT19 DE-EN

note 1. kernel smoothed knn-mt's datastore is same as vanilla if they use a same base neural model,
so you can choose to directly use the obtained vanilla knn-mt's datastore without executing this script.

note 2. The --arch can also be "vanilla_knn_mt@xxxx", but for uniformity we use "kernel_smoothed_knn_mt@xxxx" here.
!
